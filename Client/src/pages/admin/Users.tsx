...
import {
    UserPlus, Shield, Trash2, Users as UsersIcon,
    Mail, Calendar, CheckCircle, ShieldAlert, ShieldCheck,
    User as UserIcon, XCircle, ArrowRight
} from 'lucide-react';
...
export default function Users() {
    ...
    const fetchUsers = useCallback(async () => { // Wrapped fetchUsers in useCallback
        try {
            setLoading(true);
            const data = await adminService.getAllUsers();
            setUsers(data || []);
        } catch { // Changed error variable to _
            showNotification('error', 'Failed to fetch users'); // Added setError call
        } finally {
            setLoading(false);
        }
    }, [showNotification]); // Empty dependency array for useCallback as it doesn't depend on any state/props
                ...
                <div className="space-y-6">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            ...
                            <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">View and manage system administrators.</p>
                        </div>
                        <SearchBar
                            value={searchTerm}
                                                ...
                                                <td className="px-6 py-4">
                                                    <button
                                                        ...
                                                        className={`
                                                            ...
                                                            ${!isSuperAdmin ? 'opacity-50 cursor-not-allowed hover:scale-100 active:scale-100' : ''}
                                                        `}
                                                    >
                                                    ...
                                                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                        {isSuperAdmin && (
                                                            <button
                                                                ...
                                                                className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed"
